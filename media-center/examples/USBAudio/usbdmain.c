...
void TIMER0_IRQHandler(void) 
{
  ...
  if ((Tick++ & 0x03FF) == 0) {             /* On every 1024th Tick */
	...
	if (joy == KBD_SELECT){
		...
		NVIC_DisableIRQ(USB_IRQn);		// Disabled the USB interrupt
		USB_Connect(FALSE);
		USB_Reset();
...
extern int audio_main (void)
{
  volatile uint32_t pclkdiv, pclk;
	/* ---------- SPLASHSCREEN AND EXIT FUNCTIONALITY ----------- */
	KBD_Init();
	GLCD_Init();
	GLCD_Clear  (Yellow);
  ...
  GLCD_DisplayString(2, 0, __FI, " SPICY AUDIO PLAYER ");
	GLCD_SetBackColor(Yellow);
	GLCD_SetTextColor(Blue);
	GLCD_DisplayString(5, 0, __FI, "    Push to Exit    ");
